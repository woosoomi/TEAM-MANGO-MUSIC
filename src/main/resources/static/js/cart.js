...
function changeQuantity(amount, element) {
	...
	//var cartItemId = document.getElementsByClassName('test').getAttribute('data-cartitemid');
	var relatedTextbox = element.parentElement.querySelector('.test');
    var cartItemId = relatedTextbox.value;

        console.log(cartItemId);

	if (newQty >= 1) {
		cartItemQtyElement.value = newQty;
		console.log(cartItemQtyElement.value);
		// AJAX를 사용하여 서버로 카트 아이템 수량을 업데이트합니다.
		// 이 부분은 실제로 서버로 업데이트 요청을 보내는 코드로 채워야 합니다.
		// 예를 들어 jQuery.ajax 또는 fetch API를 사용할 수 있습니다.
		$.ajax({
